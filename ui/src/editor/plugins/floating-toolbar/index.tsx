...
import { Toolbar } from '@/editor/plugins/floating-toolbar/toolbar';
import { useTargetRect } from '@/editor/plugins/floating-toolbar/use-target-rect';
import { TextFormatType, useTextFormat } from '@/editor/plugins/floating-toolbar/use-text-format';

const TextFormatTypesToRender: Array<TextFormatType> = [
  'bold',
  'italic',
  'strikethrough',
  'underline',
...
function TextFormatToolbar({ editor }: { editor: LexicalEditor }) {
  ...
  const cells: /* Array<CellProps> */ Parameters<typeof Toolbar>[0]['cells'] =
    TextFormatTypesToRender.map((type) => ({
...
function FloatingContainer({
  ...
  if (containerX + contentWidth + RIGHT_GAP > anchorElementRect.left + anchorElementRect.width) {
    containerX +=
      anchorElementRect.left + anchorElementRect.width - containerX - contentWidth - RIGHT_GAP;
  }
  ...
  return createPortal(
    <div
      ...
      style={{ transform: `translate3d(${containerX}px, ${containerY}px, 0)` }}
    >
